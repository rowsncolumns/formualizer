...
fn direct_range_and_transitive_dependents() {
    ...
    assert!(
        closure.contains(&("Sheet1".to_string(), 1, 2)),
        "direct dependent missing: {closure:?}"
    );
    assert!(
        closure.contains(&("Sheet1".to_string(), 1, 3)),
        "transitive dependent missing: {closure:?}"
    );
    assert!(
        closure.contains(&("Sheet1".to_string(), 7, 3)),
        "range dependent missing: {closure:?}"
    );
    assert!(
        !closure.contains(&("Sheet1".to_string(), 8, 3)),
        "unrelated range must not appear: {closure:?}"
    );
    assert!(
        !closure.contains(&("Sheet1".to_string(), 1, 1)),
        "the root itself must not appear: {closure:?}"
    );
}
...
fn closure_is_read_only_and_multi_source() {
    ...
    assert!(
        closure.contains(&("Sheet1".to_string(), 5, 6)),
        "{closure:?}"
    );
...
fn spill_region_of_reports_committed_projections() {
    ...
    assert!(
        engine.cell_spill_region("Sheet1", 2, 2).is_empty(),
        "a projection anchors nothing"
    );
    assert!(
        engine.cell_spill_region("Sheet1", 9, 9).is_empty(),
        "non-anchor cell is empty"
    );
    assert!(
        engine.cell_spill_region("Nope", 1, 1).is_empty(),
        "unknown sheet is empty"
    );
}
